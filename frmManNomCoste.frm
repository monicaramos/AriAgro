...
Begin VB.Form frmManNomCoste 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Denominación de Costes"
   ClientHeight    =   8940
   ClientLeft      =   45
   ClientTop       =   30
   ClientWidth     =   8910
   Icon            =   "frmManNomCoste.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   8940
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.TextBox text1 
         ...
         MaxLength       =   2
         TabIndex        =   0
         Tag             =   "Código|N|N|||nombcoste|codcoste|00|S|"
         Text            =   "12"
         Top             =   315
         Width           =   450
      End
      Begin VB.TextBox text1 
         ...
         MaxLength       =   40
         TabIndex        =   1
         Tag             =   "Nombre|T|N|||nombcoste|denominacion|||"
   ...
   Begin VB.Frame FrameAux0 
      ...
      Begin VB.TextBox txtAux 
         ...
         MaxLength       =   16
         TabIndex        =   12
         Tag             =   "Codigo|N|N|||ctacoste|codcoste|00|S|"
         Text            =   "codigo"
         Top             =   2565
         Visible         =   0   'False
         Width           =   375
      ...
      Begin VB.TextBox txtAux 
         ...
         MaxLength       =   10
         TabIndex        =   2
         Tag             =   "Cuenta Contable|T|N|||ctacoste|codmacta||N|"
         Text            =   "codmacta"
         Top             =   2565
         Visible         =   0   'False
         Width           =   1035
      ...
      Begin VB.TextBox txtAux 
         ...
         MaxLength       =   16
         TabIndex        =   9
         Tag             =   "Linea|N|N|||ctacoste|numlinea|00|S|"
         Text            =   "lin"
         Top             =   2565
         Visible         =   0   'False
         Width           =   240
      ...
      Begin MSDataGridLib.DataGrid DataGridAux 
         Bindings        =   "frmManNomCoste.frx":000C
         Height          =   4545
         Index           =   0
         Left            =   135
         TabIndex        =   14
   ...
   End
End
Attribute VB_Name = "frmManNomCoste"
...
Private WithEvents frmNomCost As frmBasico2
Attribute frmNomCost.VB_VarHelpID = -1
Private WithEvents frmC As frmCal 'calendario fecha
Attribute frmC.VB_VarHelpID = -1
Private WithEvents frmCta As frmCtasConta 'cuentas contables
...
Dim numNivel As String
    ...
    Select Case Index
        Case 0 'Cuentas contables
            ...
            frmCta.CodigoActual = txtAux(1).Text
            numNivel = DevuelveDesdeBDNew(cConta, "empresa", "numnivel", "codempre", vParamAplic.NumeroConta, "N")
'            frmCta.NumDigit = DevuelveDesdeBDNew(cConta, "empresa", "numdigi" & numNivel - 1, "codempre", vParamAplic.NumeroConta, "N")
'            frmCta.ConApudirec = True
'[Monica] 07/10/2009 He quitado las 2 insts anteriores y la he sustiuido por la siguiente
            frmCta.NumDigit = 0
            frmCta.Show vbModal
            Set frmCta = Nothing
            PonerFoco txtAux(1)
...
Dim tabla As String
    ...
    Select Case Index
        ...
        Case 0 'CUENTAS CONTABLES
            '[Monica]20/07/2018: se iba a buscar a la contabilidad antigua. CORREGIDO
            If vParamAplic.ContabilidadNueva Then
                Sql = "SELECT ctacoste.codcoste, ctacoste.numlinea, ctacoste.codmacta, ariconta" & vParamAplic.NumeroConta & ".cuentas.nommacta "
                Sql = Sql & " FROM ctacoste, ariconta" & vParamAplic.NumeroConta & ".cuentas "
            Else
                Sql = "SELECT ctacoste.codcoste, ctacoste.numlinea, ctacoste.codmacta, conta" & vParamAplic.NumeroConta & ".cuentas.nommacta "
                Sql = Sql & " FROM ctacoste, conta" & vParamAplic.NumeroConta & ".cuentas "
            End If
            
            If enlaza Then
                Sql = Sql & ObtenerWhereCab(True)
            Else
                Sql = Sql & " WHERE ctacoste.codcoste = -1"
            End If
            If vParamAplic.ContabilidadNueva Then
                Sql = Sql & " and ctacoste.codmacta = ariconta" & vParamAplic.NumeroConta & ".cuentas.codmacta "
            Else
                Sql = Sql & " and ctacoste.codmacta = conta" & vParamAplic.NumeroConta & ".cuentas.codmacta "
            End If
            Sql = Sql & " ORDER BY ctacoste.numlinea"
...
'Cuentas contables
    txtAux(2).Text = RecuperaValor(CadenaSeleccion, 1) 'codmacta
...
Private Sub frmNomCost_DatoSeleccionado(CadenaSeleccion As String)
Dim CadB As String
Dim Aux As String
    ...
    If CadenaSeleccion <> "" Then
        HaDevueltoDatos = True
        Screen.MousePointer = vbHourglass
        CadB = "codcoste = " & RecuperaValor(CadenaSeleccion, 1)
        CadenaConsulta = "select * from " & NombreTabla & " WHERE " & CadB & " " & Ordenacion
        PonerCadenaBusqueda
        Screen.MousePointer = vbDefault
    End If
...
Private Sub MandaBusquedaPrevia(CadB As String)
    ...
    Set frmNomCost = New frmBasico2
    
    AyudaNomCostePrev frmNomCost, , CadB
    
    Set frmNomCost = Nothing
...
Dim CPostal As String, desProvi As String, desPais As String
    ...
    If Not Adoaux(0).Recordset.EOF Then
        ...
        ComprobarCuentas Str(Adoaux(0).Recordset!codCoste)
    End If
...
Dim Cad As String, Indicador As String
    ...
    Cad = "(codcoste=" & DBSet(Text1(0).Text, "T") & ")"
...
Dim vWhere As String
    ...
    vWhere = " WHERE codcoste=" & DBSet(Data1.Recordset!codCoste, "N")
        
    ' ***** elimina les llínies ****
    conn.Execute "DELETE FROM ctacoste " & vWhere
...
Dim Eliminar As Boolean
    ...
    Select Case Index
        Case 0 'envases
            Sql = "¿Seguro que desea eliminar la Cta Contable?"
            Sql = Sql & vbCrLf & "Cuenta : " & Adoaux(Index).Recordset!Codmacta
            If MsgBox(Sql, vbQuestion + vbYesNo) = vbYes Then
                Eliminar = True
                Sql = "DELETE FROM ctacoste "
    ...
    Select Case Index
        Case 2 ' cuentas contables
            If txtAux(Index) <> "" Then
                If Len(txtAux(Index).Text) = vEmpresa.DigitosUltimoNivel Or InStr(1, txtAux(Index), ".") Then
                    txtAux2(Index).Text = PonerNombreCuenta(txtAux(Index), Modo)
                Else
                    If Len(txtAux(Index).Text) = vEmpresa.DigitosNivelAnterior Then
                        txtAux2(Index).Text = DevuelveDesdeBDNew(cConta, "cuentas", "nommacta", "codmacta", txtAux(Index).Text, "T")
                    Else
                        txtAux2(Index).Text = ""
                        MsgBox "Longitud de cuenta incorrecta. Reintroduzca.", vbExclamation
                    End If
                End If
                
                If txtAux2(Index).Text <> "" Then ComprobarRestoCuentas txtAux(0).Text, txtAux(1).Text, txtAux(2).Text
            End If
    End Select
End Sub
...
Dim vFact As Byte, vDocum As Byte
    ...
    b = ComprobarRestoCuentas(txtAux(0).Text, txtAux(1).Text, txtAux(2).Text)
    If Not b Then Exit Function
...
Dim tots As String
    ...
    Select Case Index
        Case 0 'cuentas contables
            ...
            tots = "N||||0|;" 'codcoste
            tots = tots & "S|txtAux(1)|T|Linea|1000|;S|txtAux(2)|T|Cuenta|1400|;S|btnBuscar(0)|B|||;"
            tots = tots & "S|txtAux2(2)|T|Denominación|5250|;"
...
Private Function ComprobarRestoCuentas(Coste As String, Linea As String, Cuenta As String) As Boolean
Dim Sql As String
Dim Rs As ADODB.Recordset
    
    ComprobarRestoCuentas = False

    Sql = "select count(*) from ctacoste where codcoste = " & DBSet(Coste, "N") & " and numlinea <> " & DBSet(Linea, "N")
    Sql = Sql & " and codmacta = '" & Trim(Cuenta) & "'"
    If DevuelveValor(Sql) Then
        MsgBox "Esta cuenta ya está en otro registro. Revise.", vbExclamation
        Exit Function
    End If

    If Len(Cuenta) = vEmpresa.DigitosNivelAnterior Then
        Sql = "select count(*) from ctacoste where codcoste = " & DBSet(Coste, "N") & " and numlinea <> " & DBSet(Linea, "N")
        Sql = Sql & " and codmacta like '" & Trim(Cuenta) & "%'"
        
        If DevuelveValor(Sql) <> 0 Then
            MsgBox "Hay cuentas del último nivel que pertenecen a este grupo. Revise.", vbExclamation
            Exit Function
        Else
            ComprobarRestoCuentas = True
        End If
    
    End If
    If Len(Cuenta) = vEmpresa.DigitosUltimoNivel Then
        Sql = "select count(*) from ctacoste where  codcoste = " & DBSet(Coste, "N") & " and numlinea <> " & DBSet(Linea, "N")
        Sql = Sql & " and codmacta = '" & Mid(Cuenta, 1, vEmpresa.DigitosNivelAnterior) & "'"
        
        If DevuelveValor(Sql) <> 0 Then
            MsgBox "Hay cuentas del nivel anterior que contiene esta cuenta. Revise.", vbExclamation
            Exit Function
        Else
            ComprobarRestoCuentas = True
        End If
...
Private Function ComprobarCuentas(Coste As String) As Boolean
Dim Sql As String
Dim Rs As ADODB.Recordset
Dim b As Boolean
    ...
    Sql = "select numlinea, codmacta from ctacoste where codcoste = " & DBSet(Coste, "N")
    ...
    While Not Rs.EOF And b
        b = ComprobarRestoCuentas(Coste, CStr(Rs!NumLinea), CStr(Rs!Codmacta))
        Rs.MoveNext
    Wend
    
    If b Then ComprobarCuentas = True
    Exit Function
