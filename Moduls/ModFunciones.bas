...
Public Sub AyudaClientes(frmBas As frmBasico, Optional CodActual As String)

    frmBas.CadenaTots = "S|txtAux(0)|T|Código|800|;S|txtAux(1)|T|Descripción|3800|;"
    frmBas.CadenaConsulta = "SELECT clientes.codclien, clientes.nomclien "
    frmBas.CadenaConsulta = frmBas.CadenaConsulta & " FROM "
    If vParamAplic.BDDestino <> "" Then
        frmBas.CadenaConsulta = frmBas.CadenaConsulta & vParamAplic.BDDestino & ".clientes "
    Else
        frmBas.CadenaConsulta = frmBas.CadenaConsulta & "clientes "
    End If
    frmBas.CadenaConsulta = frmBas.CadenaConsulta & " WHERE (1=1) "
    frmBas.Tag1 = "Código|N|N|0|999999|clientes|codclien|000000|S|"
    frmBas.Tag2 = "Descripción|T|N|||clientes|nomclien|||"
    frmBas.Maxlen1 = 6
    frmBas.Maxlen2 = 40
    frmBas.DeConsulta = True
    frmBas.tabla = vParamAplic.BDDestino & ".rsocios"
    frmBas.CampoCP = "codclien"
    frmBas.Report = "rManClientes.rpt"
...
Public Sub AyudaDestinos(frmBas As frmBasico, Optional CodActual As String)

    frmBas.CadenaTots = "S|txtAux(0)|T|Código|800|;S|txtAux(1)|T|Descripción|3800|;"
    frmBas.CadenaConsulta = "SELECT destinos.coddesti, destinos.nomdesti "
    frmBas.CadenaConsulta = frmBas.CadenaConsulta & " FROM " & vParamAplic.BDDestino & ".destinos "
    frmBas.CadenaConsulta = frmBas.CadenaConsulta & " WHERE codclien = " & DBSet(CodActual, "N")
    frmBas.Tag1 = "Código|N|N|0|999|destinos|coddesti|000|S|"
    frmBas.Tag2 = "Descripción|T|N|||destinos|nomdesti|||"
    frmBas.Maxlen1 = 6
    frmBas.Maxlen2 = 40
    frmBas.DeConsulta = True
    frmBas.tabla = vParamAplic.BDDestino & ".destinos"
    frmBas.CampoCP = "coddesti"
    frmBas.Report = "rManDestinos.rpt"
    frmBas.Caption = "Destinos"
    frmBas.CodigoActual = 0
    If CodActual <> "" Then frmBas.CodigoActual = CodActual
    frmBas.DatosADevolverBusqueda = "0|1|"
    frmBas.Show vbModal
...
Public Function EsAlbaranBIO(cadwhere As String) As Boolean
Dim Sql As String


    Sql = "select count(*) from (albaran inner join albaran_variedad on albaran.numalbar = albaran_variedad.numalbar) inner join variedades on albaran_variedad.codvarie = variedades.codvarie "
    Sql = Sql & " where variedades.tipovarie2 = 1 and " & cadwhere
    
    EsAlbaranBIO = (TotalRegistros(Sql) > 0)
...
eLimpiarTemporal:
    MuestraError Err.Number, "Limpiar Temporal", Err.Description
End Function

Public Function CargarTemporalPalets(vAlbaran As String, ImpDesdePalets As Boolean) As Boolean
Dim Sql As String
Dim vPedido As Long
Dim SQLinsert As String

    On Error GoTo eCargarTemporalPalets
    
    
    CargarTemporalPalets = False
    
    '[Monica]16/10/2018:
    Sql = "select numpedid from albaran where numalbar = " & DBSet(vAlbaran, "N")
    vPedido = DevuelveValor(Sql)
        
        
    '[Monica]20/12/2018: para el caso de que se imprima el palet en la misma linea
    If ImpDesdePalets Then
...
'numpalet,numlinea,codvarie,codmarca,codforfait,categori,numcajas,pesobrut,pesoneto
        
        '                                             albaran,  palet,    numlinea, codvarie,  codmarca, codforfait, categori,codcalid,  numcajas, pesobrut ,pesoneto
        SQLinsert = "insert into tmpinformes (codusu, importe1, importe2, importe3, importe4,  importe5, nombre1,    nombre2, importeb1, importeb2,importeb3,importeb4, precio1) "
        
    
    Else
        
        Sql = " select " & DBSet(vUsu.Codigo, "N") & "," & DBSet(vAlbaran, "N") & ", pp.numpalet,vv.numcajas,vv.pesobrut,vv.pesoneto "
        Sql = Sql & " from " & vParamAplic.BDSat & ".palets_variedad vv, " & vParamAplic.BDSat & ".palets pp "
        Sql = Sql & " where pp.numpedid = " & vPedido
        Sql = Sql & " and pp.numpalet = vv.numpalet "
        
        '                                             albaran,  palet,    cajas,    pesobruto, pesonetoneto
        SQLinsert = "insert into tmpinformes (codusu, importe1, importe2, importe3, importe4,  importe5) "
    ...
    CargarTemporalPalets = True
    Exit Function
    
eCargarTemporalPalets:
    MuestraError Err.Number, "Cargar Temporal Palets", Err.Description
End Function
